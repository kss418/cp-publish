...
자세한 규칙은 `references/path-rules.md`를 봅니다. Codeforces contest 종류, 정확한 제목 override, `Others` 그룹 alias, 라운드 번호 추출 규칙은 `references/codeforces-contest-rule-map.json`에서 관리합니다.
...
빠진 의존성이 있으면 Codex가 설치 계획을 보여주고 사용자에게 설치해도 되는지 먼저 묻습니다. 승인된 경우에만 `scripts/init/install_dependencies.py`를 실행합니다.
...
풀이를 올릴 GitHub 저장소 설정도 스킬 실행 중 Codex가 확인합니다. 설정 파일이 없거나, 현재 publish하려는 플랫폼 route가 없으면 Codex가 필요한 값을 물어보고 `scripts/init/configure_repos.py`로 설정합니다.
...
plan을 적용할 때는 `scripts/cp_publish/apply_plan.py`를 사용합니다. 기본적으로 `needs_confirmation`이 `true`인 plan은 실패합니다.
...
python scripts/api/codeforces_results.py contest --contest-id 2061 --user <codeforces_handle> --output C:\path\to\results.json
...
scripts/init/check_dependencies.py    # 의존성 확인
scripts/init/install_dependencies.py  # 승인 후 의존성 설치
scripts/init/install_gh_user.py       # Linux 사용자 범위 GitHub CLI 설치
scripts/init/configure_repos.py       # 저장소 라우팅 설정
scripts/init/github_integration.py    # GitHub 인증, commit, push 보조
scripts/cp_publish/                  # detection, path, metadata, planning support modules
scripts/cp_publish/plan_publish.py   # publish 전 dry-run 계획 CLI
scripts/cp_publish/apply_plan.py     # plan 기반 copy/move 및 README 갱신
